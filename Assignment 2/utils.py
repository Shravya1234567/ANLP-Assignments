...
class CreateDataset(Dataset):
    def __init__(self, cut_off_freq = 2, in_vocab_word2idx = None, in_vocab_idx2word = None, out_vocab_word2idx = None, out_vocab_idx2word = None, is_test = False):
        ...
        if is_test:
            ...
            self.in_vocab_idx2word = in_vocab_idx2word
            ...
            self.in_vocab_idx2word = {0: '<PAD>', 1: '<UNK>', 2: '<SOS>', 3: '<EOS>'}
        ...
        if not is_test:
            ...
            self.train_en, self.train_fr = self.get_tokens('data/train.en', 'data/train.fr')
            self.val_en, self.val_fr = self.get_tokens('data/dev.en', 'data/dev.fr')
        self.test_en, self.test_fr = self.get_tokens('data/test.en', 'data/test.fr')
        ...
        if not is_test:
            ...
            np.save('in_vocab_idx2word.npy', self.in_vocab_idx2word)
        ...
        for sentence in self.train_en:
            for word in sentence:
                if word not in self.in_vocab_word2idx:
                    ...
                    self.in_vocab_idx2word[idx] = word
